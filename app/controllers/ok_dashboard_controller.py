...
#     # ✅ Fetch from Brand table instead of Contract.items
#     brands = Brand.query.with_entities(Brand.name).all()

#     for (name,) in brands:
#         if name:
#             brands_set.add(name.strip())

#     # Perform case-insensitive filtering (UNCHANGED LOGIC)
#     if term:
#         result = [b for b in brands_set if term in b.lower()]
#     else:
...
#     if term:
#         query = query.filter(
#             Category.value.ilike(f"%{term}%")
#         )

#     categories = query.order_by(Category.value).limit(20).all()

#     return jsonify([c.value for c in categories])
...
# from app.forms.contract_form import ContractForm
...
#     # Get contract_date and convert to date
...
#     contracts_paginated = contract_repository.get_contracts_filtered_paginated(
...
#     return redirect(url_for("dashboard.admin_brand_manage_page"))
# # =====================================================
# # Core logic: upload categories from sellers
# # =====================================================
...
#     existing = {
#         c.value for c in Category.query.with_entities(Category.value).all()
#     }
...
#         db.session.add(
#             Category(
#                 value=category_name[:255],  # unique key
#                 text=category_name          # display text
#             )
#         )
#         inserted += 1
...
#     return len(found), inserted
# # =====================================================
# # Helper: extract EXACT categories from sellers
# # =====================================================

# def extract_exact_categories_from_sellers():
#     """
#     Returns a set of category names EXACTLY as stored in Seller table
#     (only trims leading/trailing spaces).
#     """
#     categories = set()

#     seller_categories = db.session.query(Seller.category_name).distinct().all()
...
#     return categories
# # =====================================================
# # Upload Categories from Sellers (ADMIN ACTION)
# # =====================================================

# @dashboard_bp.route("/admin/categories/upload-from-sellers", methods=["POST"])
# @login_required
# @admin_required
# def admin_upload_categories_from_sellers_exact():

#     total_found, inserted = upload_categories_from_sellers_exact()

#     flash(
#         f"Upload completed. Found {total_found} categories in sellers, "
...
# @dashboard_bp.route("/dashboard/upload-excel", methods=["GET", "POST"])
# @login_required
# def upload_excel():
...
# #             if file_type == "contract":
# #                 folder = current_app.config["CONTRACT_FOLDER"]
# #             else:
# #                 folder = current_app.config["SELLER_FOLDER"]

# #             save_path = os.path.join(folder, secure_filename(unique_name))
# #             file.save(save_path)

# #             flash("Excel uploaded successfully!", "success")
# #             return redirect(request.url)

# #         flash("Only .xlsx or .xls files allowed", "danger")
